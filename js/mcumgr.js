...
class MCUTransportSerial extends MCUTransport {
    constructor(di = {}) {
        ...
        this.disc_id = 0;
        console.log("new MCUTransportSerial");
    ...
    async connect(filters) {
        try {
            ...
            this._logger.info(`connecting to device ${this.name}, reconnecting: ${this._reconnecting}`);
            ...
            */
            ...
            if (this._port) {
                const disconnecting_handler = async (event) => {
                    console.log(`disc ${this.disc_id}`)
                    this._logger.info(event);
                    await this.disconnect().catch(reason=>{});
                    console.log(`disc ${this.disc_id++}`)
                }
                this._port.addEventListener('disconnect', disconnecting_handler);
                this._reconnect_msecs = 0;
                this._connect(0);
        ...
        return Promise.reject(`Failed to connect device: ${this.name}`)
    }

    async _start_reconnection(filters) {
        setTimeout(async () => {
            if (!this._reconnecting) {
                return Promise.resolve();
            }
            ...
            try {
                ...
                console.log(`disconnect() in reconnect()`)
                await this.disconnect().catch(reason => console.log(reason));
                await this.connect(filters);

            } catch (error) {
                this._logger.error(error);
                await this.disconnect().catch(reason=>{});
            }
            this._start_reconnection(filters);
        }, RECONNECT_DELAY);
    }
    ...
    _connect(timeout) {
        setTimeout(async () => {
            try {
                this._connecting(); /* change the UI to connecting*/
                const options = {
                    baudRate: 115200
                };
                await this._port.open(options);
                this._logger.info(`port opened for _connect, reconnecting: ${this._reconnecting}`);
                ...
                this._readIncoming().catch(reason => console.log(`_readIncoming error!! ${reason}`)); /* start the reading async */
                this._onConnect(); /* trigger the mcumgr onConnect*/
            } catch (error) {
                this._logger.error(error);
                await this.disconnect().catch(reason=>{});
            }
    ...
    async _disconnected() {
        ...
        console.log("MCUTransportSerial _disconnected. reconnecting:", this._reconnecting);
        if (!this._reconnecting) {
            super._disconnected()
        }
        this._port = null;
    ...
    async disconnect() {
        ...
        if (this._writer) {
            await this._writer.abort().catch(reason => { console.log(reason); });
            this._writer.releaseLock();
        }
        if (this._port) {
            await this._port.close().catch(reason => { console.log(reason); });
        }
        await this._disconnected().catch(reason => { });
    }
    get name() {
        return "Serial";
    }
...
class MCUManager {
    ...
    reconnect(filters) {
        if (this._transport) {
            this._transport._reconnecting = true;
            return this._transport._start_reconnection(filters);
        }
    }
    ...
    onRecoveryModeConnected() {
        console.log("onRecoveryModeConnected");
        this._transport._reconnecting = false;
        this._transport._reconnect_msecs = 0;
        this._transport._reconnect_count = 0;
    }
